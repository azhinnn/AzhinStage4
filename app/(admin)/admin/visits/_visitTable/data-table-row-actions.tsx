...
export function DataTableRowActions<TData>({
  ...
  async function onStatusChange({
    ...
    if (status === "completed" && totalPaid.toFixed(2) === "0.00") {
      toast.warning("Patient has to pay at least a minimum amount", {
        duration: 6000,
      });
    } else {
      // update visit status
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          ...
          <DropdownMenuItem
            onClick={() =>
              router.push(
                `/admin/transactions/add?appid=${visit.Appointment.id}`
              )
            }>
          ...
          <DropdownMenuItem
            onClick={() =>
              router.push(
                `/admin/appointments/detail?id=${visit.Appointment.id}`
              )
            }>
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter className="gap-2">
            ...
            <Button asChild className="w-20">
              <Link href={`/admin/visits/add?id=${visit.Appointment.id}`}>
                Add
              </Link>
            </Button>
          </AlertDialogFooter>
      ...
      <AlertDialog open={isPaymentOpen} onOpenChange={setIsPaymentOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter className="gap-2">
            ...
            <Button asChild className="w-20">
              <Link
                href={`/admin/transactions/add?appid=${visit.Appointment.id}&vid=${visit.id}&status=${newStatus}`}>
                Add
              </Link>
            </Button>
          </AlertDialogFooter>
